...
def login_user(request):
    if request.method != 'POST':
        ...
        return JsonResponse(
            {"status": "error", "message": msg}, status=405)
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": msg}, status=500)
...
def get_dealerships(request, state="All"):
    ...
    try:
        ...
        return JsonResponse(
            {"status": 500, "message": msg}, status=500)
...
def get_cars(request):
    ...
    try:
        with open(
            CAR_RECORDS_FILE, 'r', encoding='utf-8') as file_handle:
